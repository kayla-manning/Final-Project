...
anim_data <- tidy_int_reg_bag %>% 
  ...
  group_by(month_year, distance, house, neighborhood) %>% 
...
test_model <- lm(avg_int ~ distance, data = anim_data)
summary(test_model)

testgrid <- seq(min(anim_data$distance), max(anim_data$distance), 1)

plot(testgrid, predict(test_model, newdata = list(distance = testgrid)))
```
